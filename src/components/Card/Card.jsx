...
import {addItemToCard, removeItemFromCard} from "../../features/user/userSlice";

const Card = () => {

    const {card} = useSelector(({user}) => user);
    const dispatch = useDispatch();
    const changeQuantity = (item, quantity) => {
        dispatch(addItemToCard({...item, quantity}))
    }
    ...
    return (
        <section className={styles.cart}>
            <h2 className={styles.title}>Your card</h2>
            {!card.length ? (
                ...
                <>
                    ...
                    <div className={styles.actions}>
                        <div className={styles.total}>
                            TOTAL PRICE: {" "}
                            <span>
                                {sumBy(card.map(({quantity, price}) => quantity * price))} $
                            </span>
                        </div>
